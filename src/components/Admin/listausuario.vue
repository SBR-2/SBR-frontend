<template>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="solicitud in filteredSolicitudes" :key="solicitud.correo">
              ...
              <td>{{ solicitud.inspectorEstado }}</td>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const { result, loading, error, onError } = useQuery(GET_INSPECTOR);
          ...
          if (newValue && newValue.fichas && newValue.fichas.items) {
            solicitudes.value = newValue.fichas.items
              .map((ficha) => {
                ...
                const inspector = ficha.inspector;
                ...
                if (usuario && producto) {
                  return {
                    ...
                    inspectorNombre: inspector?.nombre || 'Sin asignar',
                    inspectorEstado: inspector?.estado || 'Desconocido',
                  };
                } else {
                  return null;
                }
      ...
      const filteredSolicitudes = computed(() => {
        ...
        return solicitudes.value.filter((solicitud) => {
          ...
          return (
            ...
            solicitud.inspectorNombre.toLowerCase().includes(term) ||
            solicitud.inspectorEstado.toLowerCase().includes(term)
          );
        });
      });
